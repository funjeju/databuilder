        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {data.attributes && (
            <Card>
              ...
              <div className="mt-6">
                <CheckboxGroup label="추천 대상" options={TARGET_AUDIENCE_OPTIONS} selectedOptions={data.attributes.targetAudience} onChange={opt => handleAttributeChange('targetAudience', data.attributes.targetAudience.includes(opt) ? data.attributes.targetAudience.filter(o => o !== opt) : [...data.attributes.targetAudience, opt])} />
              </div>
              <div className="mt-6">
                <CheckboxGroup label="추천 시즌" options={[...RECOMMENDED_SEASONS_OPTIONS, ...MONTH_OPTIONS]} selectedOptions={data.attributes.recommendedSeasons} onChange={opt => handleAttributeChange('recommendedSeasons', data.attributes.recommendedSeasons.includes(opt) ? data.attributes.recommendedSeasons.filter(o => o !== opt) : [...data.attributes.recommendedSeasons, opt])} />
              </div>
            </Card>
          )}
